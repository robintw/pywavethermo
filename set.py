...
class WaveMessageBot(sleekxmpp.ClientXMPP):
    ...
    def __init__(self):
        ...
        #message = "PUT /heatingCircuits/hc1/manualTempOverride/status HTTP:/1.0\nContent-Type: application/json\nContent-Length: 25\nUser-Agent: NefitEasy\n\n\n\n9Z/4tNthJEwU4HhAaReEFQ==n"
#"/heatingCircuits/hc1/usermode"

        # WORKS - sets to manual mode
        #message = create_message("/heatingCircuits/hc1/usermode", 'manual')

        # Works to set temperature override temperature...but doesn't actually *do* the override
        #message = create_message("/heatingCircuits/hc1/manualTempOverride/temperature", 10)

        # Does the override, if in clock mode (NOT manual mode!)
        #message = create_message("/heatingCircuits/hc1/manualTempOverride/status", 'on')

        # Works to set temperature in manual mode
        #message = create_message("/heatingCircuits/hc1/temperatureRoomManual", 24)

        #message = create_message("/heatingCircuits/hc1/usermode", 'clock')
